...
public interface OracleDistributedService<T> extends DistributedService<T, OracleAccount> {
  OracleVaultServerService getVaultServerService();
  OracleConsulServerService getConsulServerService();
  ArtifactService getArtifactService();
  ServiceInterfaceFactory getServiceInterfaceFactory();
  String getOracleImageProject(String deploymentName, SpinnakerArtifact artifact);
  ...
  default Provider.ProviderType getProviderType() {
    return Provider.ProviderType.ORACLE;
  }

  @Override
  default void resizeVersion(AccountDeploymentDetails<OracleAccount> details, ServiceSettings serviceSettings, int version, int targetSize) {
    String name = getVersionedName(version);
    String zone = serviceSettings.getLocation();
    OracleProviderUtils.resize(details, zone, name, targetSize);
  }
  ...
  default String getArtifactId(String deploymentName) {
    ...
    return String.format("projects/%s/global/images/%s",
        getOracleImageProject(deploymentName, getArtifact()),
        String.join("-", "spinnaker", artifactName, version.replace(".", "-").replace(":", "-")));
  }

  default VaultConnectionDetails buildConnectionDetails(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings, String secretName) {
    OracleVaultServerService vaultService = getVaultServerService();
  ...
  @Override
  default List<ConfigSource> stageProfiles(AccountDeploymentDetails<OracleAccount> details,
  ...
  default List<Metadata.Items> getMetadata(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings, List<ConfigSource> configSources, Integer version) {
    ...
    items = new Metadata.Items()
        .setKey("ssh-keys")
        .setValue(OracleProviderUtils.getSshPublicKey());
    metadataItems.add(items);
    ...
    OracleConsulServerService consulServerService = getConsulServerService();
  ...
  @Override
  default void ensureRunning(AccountDeploymentDetails<OracleAccount> details,
                             ResolvedConfiguration resolvedConfiguration,
    ...
    OracleAccount account = details.getAccount();
    Compute compute = OracleProviderUtils.getCompute(details);
    ...
    NetworkInterface networkInterface = new NetworkInterface()
        .setNetwork(OracleProviderUtils.ensureSpinnakerNetworkExists(details))
        .setAccessConfigs(Collections.singletonList(accessConfig));

    ServiceAccount sa = new ServiceAccount()
        .setEmail(OracleProviderUtils.defaultServiceAccount(details))
        .setScopes(getScopes());
    ...
    try {
      ...
      OracleProviderUtils.waitOnGlobalOperation(compute, project, operation);
    } catch (IOException e) {
    ...
    try {
      ...
      OracleProviderUtils.waitOnZoneOperation(compute, project, settings.getLocation(), operation);
    } catch (IOException e) {
  ...
  @Override
  default Map<String, Object> getLoadBalancerDescription(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings) {
    return new HashMap<>();
  }

  @Override
  default Map<String, Object> getServerGroupDescription(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings, List<ConfigSource> configSources) {
    OracleAccount account = details.getAccount();
    RunningServiceDetails runningServiceDetails = getRunningServiceDetails(details, runtimeSettings);
    ...
    String network = OracleProviderUtils.getNetworkName();
    ...
    String serviceAccountEmail = OracleProviderUtils.defaultServiceAccount(details);
    ...
    /* TODO(lwander): credential class cannot be serialized as-is, making this type of construction impossible
    ...
    // credentials constructor prevents us from neatly creating a deploy description with only a name supplied
    String jsonKey = null;
  ...
  @Override
  default RunningServiceDetails getRunningServiceDetails(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings) {
  ...
  default <S> URI sshTunnelIntoService(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings, SpinnakerService<S> sidecar) {
    ...
    try {
      return OracleProviderUtils.openSshTunnel(details, instances.get(0).getId(), settings);
  ...
  @Override
  default <S> S connectToInstance(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings, SpinnakerService<S> sidecar, String instanceId) {
    try {
      return getServiceInterfaceFactory().createService(OracleProviderUtils.openSshTunnel(details, instanceId, runtimeSettings.getServiceSettings(sidecar)).toString(), sidecar);
    } catch (InterruptedException e) {
      throw new DaemonTaskInterrupted(e);
    }
  }
  ...
  @Override
  default <S> S connectToService(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings, SpinnakerService<S> sidecar) {
    return getServiceInterfaceFactory().createService(sshTunnelIntoService(details, runtimeSettings, sidecar).toString(), sidecar);
  }

  @Override
  default String connectCommand(AccountDeploymentDetails<OracleAccount> details, SpinnakerRuntimeSettings runtimeSettings) {
    RunningServiceDetails runningServiceDetails = getRunningServiceDetails(details, runtimeSettings);
    Integer version = runningServiceDetails.getLatestEnabledVersion();
    if (version == null) {
  ...
  @Override
  default void deleteVersion(AccountDeploymentDetails<OracleAccount> details, ServiceSettings settings, Integer version) {
    String migName = getVersionedName(version);
    String zone = settings.getLocation();
    ...
    try {
      OracleProviderUtils.waitOnZoneOperation(compute, project, zone, compute.instanceGroupManagers().delete(project, zone, migName).execute());
    } catch (IOException e) {
    ...
    try {
      OracleProviderUtils.waitOnGlobalOperation(compute, project, compute.instanceTemplates().delete(project, instanceTemplateName).execute());
    } catch (IOException e) {
